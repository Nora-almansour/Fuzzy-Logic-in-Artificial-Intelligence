...
char rep[] = "dulr";
int notvalid1[4] = {3, 0, 0, 2 };
int notvalid2[4] = {4, 1, 3, 5 };
int notvalid3[4] = { 5, 2, 3, 5 };
int applyparam[4] = { +3, -3, -1, +1 };
int goal_block[6] = { 0, 1, 2, 3, 4, 5}; //4 indicates no tile
int maxdepth;
elementstruct* top;
...
int main()
{
  ...
  printf("\nPlease Enter the initial state of the game \n"
         " [Represent tiles with numbers 1 to 5, and the blank space as 'x'.\n"
         " Start writing them from left to right for each row. Start from the topmost row to the bottommost row.\n"
         " Your final string will look similar to this '1 4 2 3 x 5'.\n"
  ...
  while(i<6)
    ...
    if (chr==32) continue;
    if (chr=='x') block[i] = 6;
    else if (chr >= '1' && chr <= '6') block[i] = chr - '1';
    else { printf("Invalid Input. Example of valid input...1 2 3 4 5 x.", chr); return 1; }
    i++;
    }

  fgetc(stdin); //flush out the end of line character

  printf("\n Now Enter the Goal State in a similar way. (Typical. 1 2 3 4 5 x): ");
  ...
  while(i<6)
    ...
    chr = fgetc(stdin);
    if (chr==32) continue;
    if (chr=='x') goal_block[i] = 6;
    else if (chr >= '1' && chr <= '6') goal_block[i] = chr - '1';
    else { printf("chr=%d. Invalid Input. Example of valid input...1 2 3 4 5 x.",(int) chr); return 1; }
    i++;
    }
...
int apply (int* newstate, int* oldstate, int op)
{
  ...
  newstate[blank+applyparam[op]] = 6;
